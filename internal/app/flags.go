...
type SessionConfig struct {
	...
	InferiorTTY string
	// LogFile, if set, appends structured logs to that path (-log / --log).
	// Empty means no file logging (default).
	LogFile string
}
...
// ParseFlags turns argv (without the program name) into a SessionConfig.
// It returns flag.ErrHelp for -h/-help so the caller can exit quietly.
func ParseFlags(args []string) (SessionConfig, error) {
	...
	logFile := fs.String("log", "", "append structured logs to file (default: no file logging)")
	fs.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "  -log file         Append structured logs to file (off by default)\n")
		...
		fmt.Fprintf(os.Stderr, "  gdbforge --log gdbforge.log ./hello\n")
