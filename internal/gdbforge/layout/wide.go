...
func BuildWide(panes Panes) *termforge.WidgetTree {
	...
	tree.Split(termforge.Vertical, panes.Threads)
...
func applyWideRatios(root *termforge.Node) {
	...
	if bottom == nil || bottom.Type != termforge.NodeSplit || bottom.Dir != termforge.Vertical {
		return
	}
	bottom.Ratio = wideGdbRatio

	side := bottom.Second // (Threads|Callstack) over Breakpoints
	if side == nil || side.Type != termforge.NodeSplit || side.Dir != termforge.Horizontal {
		return
	}
