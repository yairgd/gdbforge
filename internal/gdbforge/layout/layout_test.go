...
func TestBuildDefaultRatios(t *testing.T) {
	...
	right := root.Second
	if right == nil || right.Dir != termforge.Horizontal {
	...
	bottom := right.Second
	if bottom == nil || bottom.Dir != termforge.Horizontal {
...
func TestBuildPanelsStructure(t *testing.T) {
	...
	root := tree.Root()
	if root == nil || root.Dir != termforge.Vertical {
	...
	if bottom.Second == nil || bottom.Second.Type != termforge.NodeLeaf {
		t.Fatal("expected Breakpoints leaf under pair")
	}
	pair := bottom.First
	if pair == nil || pair.Dir != termforge.Vertical {
		t.Fatal("expected Threads|Callstack vertical split")
	}
	if pair.First == nil || pair.First.Type != termforge.NodeLeaf {
		t.Fatal("expected Threads leaf on left")
	}
	if pair.Second == nil || pair.Second.Type != termforge.NodeLeaf {
...
func TestBuildClassicStructure(t *testing.T) {
	...
	if root.First == nil || root.First.Type != termforge.NodeLeaf {
		t.Fatal("expected Code leaf")
	}
	if root.Second == nil || root.Second.Type != termforge.NodeLeaf {
		t.Fatal("expected GDB leaf")
	}
	// No nested Output column.
	if root.First.Type == termforge.NodeSplit || root.Second.Type == termforge.NodeSplit {
		t.Fatal("classic must not nest further splits")
	}
	leaves := termforge.CollectLeaves(root)
	if len(leaves) != 2 {
...
func TestBuildWideStructure(t *testing.T) {
	...
	root := tree.Root()
	if root == nil || root.Dir != termforge.Horizontal {
	...
	top := root.First
	if top == nil || top.Dir != termforge.Vertical {
	...
	bottom := root.Second
	if bottom == nil || bottom.Dir != termforge.Vertical {
	...
	side := bottom.Second
	if side == nil || side.Dir != termforge.Horizontal {
	...
	if side.Second == nil || side.Second.Type != termforge.NodeLeaf {
