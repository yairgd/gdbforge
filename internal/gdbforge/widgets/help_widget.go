...
func buildHelpLines() []string {
	return []string{
		...
		"        there (TUI apps need a real VT — :b io is only a line console).",
		...
		"    Go TUI / external port: :lua dlv_port [port] [extra args…]",
		"        (alias :lua dlv_ext_port) — headless dlv + connect",
		"",
		"=== External terminal (program stdio) ===",
		"",
		"    Why: :b io is a line console, not a full VT. TUI inferiors",
		"    (gdbforge itself, htop, games, curses apps) need a real terminal.",
		"",
		"    Quick switch (both backends):",
		"      :set inferior-tty           open terminal + route stdio there",
		"      :set inferior-tty internal  back to :b io",
		"      :lua external_tty           same via Lua",
		"",
		"    GDB vs Delve:",
		"      GDB  — -inferior-tty-set applies live; session stays up.",
		"             Also: :lua terminal_debug / gdbserver_tui.",
		"      Delve — --tty is only at spawn time; :set inferior-tty",
		"             restarts `dlv exec` (BPs re-applied when possible).",
		"             For Go TUIs prefer :lua dlv_port [port]:",
		"               1) start with: gdbforge -g dlv -- ./prog",
		"               2) :lua dlv_port          # or dlv_port 2345",
		"               3) headless dlv runs in another window;",
		...
		"    n/s/c keys and typed next/step/continue use MI -exec-* (no CLI",
