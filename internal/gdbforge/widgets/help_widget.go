...
func buildHelpLines() []string {
	return []string{
		...
		"",
		"Normal:",
		"    Default navigation mode. Global keys (n/s/c/f, Space, window focus)",
		...
		"",
		"Lua (ModeLua):",
		"    Enter via :lua snake / :lua tetris (ModeLua cell panes). All keys go to the",
		...
		"    n / N          Code: n = GDB next (like s/c/f); N = prev search. Else n/N search",
		"    h / l          move Code caret left / right (also arrows)",
		...
		"    f              GDB finish / Delve stepout (also in insert when Code focused)",
		...
		"    :N / :0            jump Code browse cursor to line N (:0 = 1)",
		"",
		"Layout:",
		...
		"    :layout wide asm   Layout plus Assembly split (classic/wide/panels: right;",
		"                       default: below Code)",
		...
		"    log [file]",
		"        Append structured logs to file (default gdbforge.log).",
		"        Also: gdbforge --log file ./prog (off by default).",
		...
		"    :lua <func> [args] gdbforge Lua command (e.g. :lua r5_debug)",
		"    :lua <func> help   script help() → :b io (also -h / --help)",
		"    :clear             clear focused clearable pane",
		"    :close             close focused pane/split (no-op if last)",
		"    :quit / :q         quit app (confirm if inferior alive; like Ctrl-D)",
		"    :quit! / :q!       force quit — no Quit-anyway question",
		...
		"    :lua <name> help        script help() → :b io (also -h / --help)",
		"    :lua jobs run in background; Ctrl-C cancels (incl. gdbforge.system)",
		"    gdbforge.print          → :b io always ([lua] prefix; copyable)",
		...
		"    gdbforge.print(...)    → :b io always ([lua] prefix)",
		...
		"    gdbforge.current_file() / current_line()  CodeWidget path + cursor",
		"    gdbforge.stop_file() / stop_line()  stop PC (━━▶) path + line",
		...
		"    Open in gVim: :lua gvim  (same --servername, new tab)",
		"    Open in terminal vim: :lua vim  (over gdbforge; resumes on :q)",
		"    Open in VS Code/VSCodium: :lua vscode  (workspace + file:line)",
		"    gdbforge.foreground(...)  suspend TUI → run argv on tty → resume",
		...
		"    n/s/c/f keys and typed next/step/continue/finish use MI -exec-* under GDB",
		...
		"      break leaf; run; :b callstack — or n/s/c/f from Code.",
		"",
		"Breakpoints (:b breakpoint):",
		"    j/k or Up/Down / click / wheel — select; Code jumps with blue cursor",
		"    (━━▶ stays on real PC). Selected row at stop PC stays green.",
		"    Enter — focus Code pane (status line green); click does not",
		"    e — toggle enable (disabled rows stay in list, removed from GDB)",
		"    d — delete from list and GDB",
		"    Row colors: breakcolor / breakdisabledcolor / breakcondcolor; green = stop PC",
		...
		"    Cmdline (: /) history: ./.gdbforge/cmdline_history.yaml (same quit/restore).",
		"",
		"Threads (:b threads):",
		...
		"    j/k Up/Down click wheel — select frame (-stack-select-frame),",
		...
		"    Cmdline history: ./.gdbforge/cmdline_history.yaml (cwd).",
		"",
		"=== Clipboard and mouse ===",
		"",
