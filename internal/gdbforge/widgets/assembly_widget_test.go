...
func asmLines(w *AssemblyWidget) []string { return w.LinesForTest() }
...
func TestAssemblyWidgetCGDBView(t *testing.T) {
	...
	if !strings.Contains(lines[0], "#8  ") {
		t.Fatalf("missing stack context: %q", lines[0])
	}
	if !strings.Contains(lines[4], "Dump of assembler code for function write:") {
		t.Fatalf("missing dump header: %q", lines[4])
	}
	pcLine := lines[6]
	...
	for i, ln := range lines {
		if strings.Contains(ln, "\x1b") {
			t.Fatalf("line %d has ANSI escape: %q", i, ln)
		}
	}
...
func TestAssemblyWidgetScrollPastPC(t *testing.T) {
	...
	if w.doc.Top != 0 {
		t.Fatalf("Top=%d want 0 so frame header is visible initially", w.doc.Top)
	}
	for i := 0; i < 25; i++ {
		w.MoveSel(1)
	}
	...
	if w.doc.Top < 1 {
		t.Fatalf("Top=%d want scrolled past frame/Dump header", w.doc.Top)
	}
}
...
func TestAssemblyWidgetBrowsePreservesScreenRow(t *testing.T) {
	...
	w.revealSel()
	if got := w.doc.CursorLine - w.doc.Top; got != 3 {
		t.Fatalf("screen row=%d want 3 (Top=%d Cursor=%d)", got, w.doc.Top, w.doc.CursorLine)
	}
...
func TestAssemblyWidgetShowsEndAndRestoresHeader(t *testing.T) {
	...
	endIdx := -1
	for i, ln := range lines {
		if strings.Contains(ln, "End of assembler dump.") {
			endIdx = i
			break
		}
	}
	...
	h := 20
	if endIdx < w.doc.Top || endIdx >= w.doc.Top+h {
		t.Fatalf("End line %d not visible (Top=%d h=%d)", endIdx, w.doc.Top, h)
	}
	for w.selIdx > 0 {
		w.MoveSel(-1)
	}
	if w.doc.Top != 0 {
		t.Fatalf("Top=%d want 0 so frame/Dump header is visible", w.doc.Top)
	}
	if !strings.Contains(lines[0], "#5  0x7ffff7e5791e in _IO_do_write") {
		t.Fatalf("frame header missing: %q", lines[0])
	}
	if !strings.Contains(lines[1], "Dump of assembler code for function _IO_do_write") {
		t.Fatalf("Dump header missing: %q", lines[1])
	}
}
...
func TestAssemblyWidgetKeepsPreambleVisible(t *testing.T) {
	...
	w.SetItems(items, "0x1000", "0x1000", "")
	if w.doc.Top != 0 {
		t.Fatalf("Top=%d want 0 so stack preamble stays visible", w.doc.Top)
	}
	lines := asmLines(w)
	if !strings.Contains(lines[0], "#0  a") {
		t.Fatalf("preamble missing: %q", lines[0])
	}
	for _, ln := range lines {
		if strings.Contains(ln, "Dump of assembler") {
			t.Fatalf("unexpected dump header on windowed view: %q", ln)
		}
	}
}
